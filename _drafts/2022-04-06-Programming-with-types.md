...
1. Designing functions that dont return values
...
2. Boolean logic and short circuits
    ...
    2. Many programming languages use the following symbols for common Boolean operations: && for **AND**, || for **OR**, and ! for **NOT**. Boolean expressions are usually described with truth tables
        ...
        ![figure 1.1.png](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/7e757da2-51fb-45af-b319-986bc8cc5f8c/figure_1.1.png)
        
    3. Short circuit evaluation
        1. Most compilers and run times perform an optimization called short circuit for Boolean expressions. Expressions of the form `a AND b` are translated to `if a then b else false`
        2. So for example, let’s say we have a function which checks for a Post comment like the followings: 
            1. `function allowPost(comment: string, userId: string): boolean {
            if ((comment == "") || (secondsSinceLastComment(userId) < 10))`
                
                `}`
                
        3. If the first operand is true, the second operand will be skipped and hence the expensive function `secondsSinceLastComment` will not be run. We prefer ordering conditions from cheapest to most expensive.
3. Common pitfalls of numerical types
    ...
    2. Integer types and overflow
        1. An **unsigned binary** encoding uses every bit to represent part of the value. A 4-bit unsigned integer, for example, can represent any value from 0 to 15. In general, an N-bit unsigned integer can represent values from 0 (all bits are 0) up to $2^N$-1 (all bits are 1)
        2. If we also want to represent **negative** numbers, we need a different encoding, which is usually **two’s complement.** In two’s complement encoding, we reserve a bit to encode the sign. Positive numbers are represented exactly as before, whereas negative numbers are encoded by subtracting their absolute value from 2N, where N is the number of bits.
    ...
    Such a situation is called an **arithmetic overflow**. The opposite situation, in which we end up with a number that is too small to represent, is called an **arithmetic** **underflow**.
        1. **Wrap around** is what the hardware usually does, as it simply discards the bits that don’t fit. For example, if we adding  1 to `1111`, the result would be `10000` but then because we only have 4 bits, it would be `0000` (1 is discarded).
        2. **Saturation** is another way to handle overflow. If the result of an operation exceeds the maximum representable value, we simply stop at the maximum.
        3. The third possibility, **error out**, is to throw an error when an overflow happens
4. Floating-point types and rounding
...
6. Building data structures with arrays and references
